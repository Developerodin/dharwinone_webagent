...
/**
 * Premium team 03 — lead cook as thesis, remaining names in a quiet rail.
 */
export function PremiumTeam03({ content, assets }: SectionComponentProps) {
      ...
      <div className="mx-auto grid max-w-6xl gap-10 @min-[768px]:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] @min-[768px]:items-end">
        <div className="min-w-0 overflow-hidden">
          {leadImage ? (
            ...
            <div aria-hidden="true" className="aspect-[4/5] w-full bg-[var(--theme-card)]" />
          )}
        </div>
        <div className="min-w-0">
          ...
          {lead ? (
            <>
              <p className={`mt-6 text-2xl ${pm.heading}`}>{lead.name}</p>
              {lead.role ? <p className={`mt-1 text-sm ${pm.body}`}>{lead.role}</p> : null}
              {lead.bio ? (
                <p className={`mt-4 text-sm leading-7 @min-[640px]:text-base ${pm.body}`}>{lead.bio}</p>
              ) : introText ? (
                <p className={`mt-4 text-sm ${pm.body}`}>{introText}</p>
              ) : null}
            </>
          ) : introText ? (
            <p className={`mt-4 text-sm ${pm.body}`}>{introText}</p>
            ...
            <ul className="mt-8 space-y-3 border-t border-[var(--theme-line)] pt-6" role="list">
              {rest.map((member) => (
                <li key={member.name} className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  ...
                  {member.role ? <span className={`text-sm ${pm.body}`}>{member.role}</span> : null}
                </li>
              ))}
            </ul>
          ) : null}
